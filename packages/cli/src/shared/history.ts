...
import { z } from 'zod';
import { safeMkdir, safePath, safeReadFile, safeRename, safeWriteFile } from '@dino/core';
import { logger } from '@dino/engine';
...
export async function saveHistoryEntry(
  ...
  // eslint-disable-next-line security/detect-non-literal-fs-filename -- safeFilepath is validated via safePath(...)
  await fs.appendFile(safeFilepath, line, 'utf-8');

  // Trim if over limit — read all lines, keep last N, rewrite
  try {
    const raw = await safeReadFile(safeFilepath, historyRoot);
    ...
    if (lines.length > options.historyLimit) {
      ...
      await safeWriteFile(tmpPath, trimmed, historyRoot);
