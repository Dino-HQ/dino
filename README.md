...
That's the whole quickstart — no account, no setup, and no Node.js: the installer puts the standalone `dino` binary on macOS or Linux (with Node.js 22+, `npm install -g @dino-hq/cli` works too; all options at [usedino.dev/docs/install](https://usedino.dev/docs/install)). Dino introspects the schema, discovers every operation, runs its full test suite, and scores the health of each endpoint. Add `--fail-on-high`, and it gates CI (exits 1 on HIGH/CRITICAL).
